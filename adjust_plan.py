...
sht = gs.open_by_key('1-LWvY2TNK-BVWMjL8c1fWG5xWYaP9quF9kVXgjEep2k')
SHEET1 = 'SEMI_DATA'
...
query_stock = """
    ...
    max_change AS (
        ...
        WHERE depot_id NOT IN (142410, 217633, 101011, 111753, 125224, 220636, 142408, 222877)
    ...
    stock_today AS (
        ...
        WHERE 
            pih.available >= 1
            AND pih.depot_id NOT IN (142410, 217633, 101011, 111753, 125224, 220636, 142408, 222877)
...
def channel(code):
    if code in ('KHO ECOM', 'ECOM2', 'ECOM', 'KHO BOXME','ECOM SG', 'ECOM HN'):
        return 'ECOM'
...
query_sales_current = """
    ...
    LEFT JOIN sale_channel sc
        ON sc.id = so.channel
    WHERE 
        so.status NOT IN ('Canceled', 'Returning', 'Failed','Returned', 'Aborted', 'CarrierCanceled', 'ConfirmReturned')
        ...
        AND YEAR(so.createdDateTime) >= '2024'
        AND MONTH(so.createdDateTime) >= '07'
    GROUP BY
        year,
        month,
        channel,
