#!/usr/bin/env python3
"""
pbi_advisor.py — Đọc model từ Power BI Desktop đang mở, dùng 3 Claude agent
phân tích measures + viết DAX mới + gợi ý cải thiện report.

Yêu cầu:
  pip install pyadomd anthropic

Cách dùng:
  1. Mở file .pbix trong Power BI Desktop
  2. python pbi_advisor.py
  3. Xem kết quả trong thư mục pbi_output/
"""
...
OUTPUT_DIR = Path(__file__).parent / "pbi_output"
...
def find_pbi_port() -> int | None:
    ...
    patterns  = [
        ...
        # Store App (Microsoft Store version)
    ...
    # Fallback: tìm qua process msmdsrv.exe đang chạy
    try:
        import psutil
...
# ══════════════════════════════════════════════════════════════════
#  STEP 1b: ĐỌC MODEL TỪ DAX STUDIO JSON EXPORT
#  (dùng khi Live Connection — không có local AS)
# ══════════════════════════════════════════════════════════════════
...
def read_model_from_csv(csv_path: str) -> dict:
    ...
    with open(csv_path, encoding="utf-8-sig", newline="") as f:
        ...
        # normalize header names (DAX Studio dùng uppercase)
        for row in reader:
...
def read_model_from_json(json_path: str) -> dict:
    ...
    # DAX Studio export format: {"model": {"tables": [{"name": ..., "measures": [...], ...}]}}
    tables_raw = (
        raw.get("model", raw).get("tables", [])   # daxstudio format
        or raw.get("tables", [])                   # flat format fallback
    )
...
def read_pbi_model(port: int) -> dict:
    ...
    with Pyadomd(conn_str) as conn:
        # Đọc tất cả measures + DAX expression
        with conn.cursor().execute("""
            SELECT
                [MEASUREGROUP_NAME]  AS tbl,
        ...
        """) as cur:
            pass  # fallback below nếu query này fail

        with conn.cursor().execute("""
            SELECT DISTINCT [DIMENSION_NAME] AS tbl
            FROM $SYSTEM.MDSCHEMA_DIMENSIONS
            WHERE [DIMENSION_IS_VISIBLE] = TRUE
        """) as cur:
            for row in cur.fetchall():
...
# ══════════════════════════════════════════════════════════════════
#  STEP 3: ĐỌC DB SCHEMA (OPTIONAL — dùng để bổ sung context)
# ══════════════════════════════════════════════════════════════════
...
def agent_1_analyze(model: dict) -> str:
    """Phân tích measures hiện có, tìm pattern và gap."""
    measures_txt = "\n".join(
...
def agent_2_write_dax(model: dict, analysis: str, db_schema: str) -> str:
    """Viết DAX cho measures mới + tối ưu measures cũ."""
    existing_names = {m["name"] for m in model["measures"]}
    sample_measures = "\n".join(
        f"  {m['name']} = {m['dax']}"
        for m in model["measures"][:20]  # 20 measures đầu làm ví dụ style
    )
...
def agent_3_report_suggestions(model: dict, analysis: str, dax_output: str) -> str:
    """Gợi ý cải thiện cấu trúc report."""
    return call_claude(
        system=textwrap.dedent("""
